...
export const showEvolutionConfirm = async (collection: Collection) => {
  ...
  const content =
    levelToDown === 0 ? (
      <>
        <Typography color="primary">{capitalize(collection.name)}</Typography> will
        disappear. Are you sure to evolve?
      </>
    ) : (
      <>
        <Typography color="primary">{capitalize(collection.name)}</Typography>
